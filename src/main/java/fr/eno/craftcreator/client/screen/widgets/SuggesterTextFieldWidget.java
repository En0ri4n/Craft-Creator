...
/**
 * Copied from {@link net.minecraft.client.gui.widget.TextFieldWidget} and adapted to fit my needs
 */
public class SuggesterTextFieldWidget extends SimpleListWidget
{
    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton)
    {
        ...
        else
        {
            ...
            {
                int i = MathHelper.floor(pMouseX) - this.textFieldX;
                if(this.bordered)
                {
                    i -= 4;
                }
    ...
    public void render(MatrixStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks)
    {
        if(this.isVisible())
        {
            ...
            if(flag1)
            {
                if(flag2)
                {
                    fill(pMatrixStack, k1, i1 - 1, k1 + 1, i1 + 1 + 9, -3092272);
                }
                else
                {
