...
public class RecipeFileUtils
{
    ...
    public static JsonObject mapToJsonObject(Map<String, Object> map)
    {
        ...
        map.forEach((s, o) ->
        {
            if(o instanceof Number)
                obj.addProperty(s, (Number) o);
            else if(o instanceof String)
                obj.addProperty(s, (String) o);
            else if(o instanceof Boolean)
                obj.addProperty(s, (Boolean) o);
            else if(o instanceof Character)
                obj.addProperty(s, (Character) o);
        });
        return obj;
    }
    ...
    private static List<ITextComponent> getModifiedRecipe(ModifiedRecipeType type, String line)
    {
        ...
        switch(type)
        {
            case REMOVED:
                ...
                comps.forEach(comp ->
                {
                    if(json.has(comp.toLowerCase()))
                    {
                        list.add(new StringTextComponent(comp + " : ").mergeStyle(TextFormatting.DARK_AQUA));
                        list.add(new StringTextComponent(json.get(comp.toLowerCase()).getAsString()).mergeStyle(TextFormatting.AQUA));
                        list.add(new StringTextComponent(""));
                    }
                });
                break;
    ...
    public static Multimap<String, ResourceLocation> getInput(IRecipe<?> recipe)
    {
        ...
        if(recipe instanceof IPureDaisyRecipe)
        {
            ...
            if(recipePureDaisy.getInput() instanceof StateIngredientTag)
                ...
                locations.put("Block", recipePureDaisy.getInput().pick(new Random()).getBlock().getRegistryName());
            return locations;
        }
    ...
    private static Multimap<String, ResourceLocation> getIngredients(NonNullList<Ingredient> ingredients)
    {
        ...
        ingredients.forEach(ingredient ->
        {
            ...
            if(json instanceof JsonArray)
                json.getAsJsonArray().forEach(je -> getAndSave(je, locations));
            else
                getAndSave(json, locations);
        });
    ...
    private static void getAndSave(JsonElement element, Multimap<String, ResourceLocation> map)
    {
        if(element.getAsJsonObject().has("item"))
            ...
            map.put("Tag", ResourceLocation.tryCreate(element.getAsJsonObject().get("tag").getAsString()));
    }
