...
public class DropdownListWidget<T extends DropdownListWidget.Entry<?>> extends SimpleListWidget
{
    ...
    public void render(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        MutableComponent title = References.getTranslate("screen.widget.dropdown_list.entry.name", ScreenUtils.truncateString(dropdownFieldWidth - 10, getMessage().getString()));
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button)
    {
        ...
        if(isFocused())
        {
            ...
            if(!ScreenUtils.isMouseHover(dropdownFieldX, dropdownFieldY + dropdownFieldHeight, (int) mouseX, (int) mouseY, dropdownFieldWidth, y1 - y0))
            {
    ...
    public void setEntry(T entry, T toReplace)
    {
        this.getEntries().set(this.getEntries().indexOf(toReplace), entry);
        trimWidthToEntries();
    }
    ...
    public static class Entries
    {
        public static ArrayList<StringEntry> getRecipeTypes(String modid)
        {
            ...
            return mod.getSupportedRecipeTypes().stream().map(recipeTypeLocation -> new StringEntry(recipeTypeLocation.toString(), new TextComponent(recipeTypeLocation.toString()))).collect(Collectors.toCollection(ArrayList::new));//Registry.RECIPE_TYPE.stream().filter(type -> CommonUtils.getRecipeTypeName(type).getNamespace().equals(modid)).map(recipeType -> new DropdownListWidget.StringEntry(CommonUtils.getRecipeTypeName(recipeType).toString(), new StringTextComponent(CommonUtils.getRecipeTypeName(recipeType).toString()))).collect(Collectors.toList());
        }

        public static ArrayList<StringEntry> getModIds()
        {
            return SupportedMods.getSupportedLoadedMods().stream().map(mod -> new StringEntry(mod.getModId(), new TextComponent(mod.getModId()))).collect(Collectors.toCollection(ArrayList::new));
        }
    }
    ...
    public static abstract class Entry<T> extends SimpleListWidget.Entry
    {
        ...
        public abstract MutableComponent getDisplayName(int index);
    }
    ...
    public static class StringEntry extends DropdownListWidget.Entry<String>
    {
        ...
        @Override
        public void render(PoseStack matrixStack, int index, int top, int left, int width, int itemHeight, int mouseX, int mouseY, boolean hovered, float partialTicks)
        {
            int color = 0xFFFFFF;
            if(hovered)
            {
